CREATE FUNCTION public.handle_post_upvote () RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER
SET
    search_path = '' AS $$
BEGIN
...
AFTER INSERT ON public.post_upvotes FOR EACH ROW
EXECUTE FUNCTION public.handle_post_upvote ();

CREATE FUNCTION public.handle_post_unvote () RETURNS trigger LANGUAGE plpgsql SECURITY definer
SET
    search_path = '' AS $$
BEGIN
...
AFTER DELETE ON public.post_upvotes FOR EACH ROW
